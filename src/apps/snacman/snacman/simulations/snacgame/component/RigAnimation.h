...
struct RigAnimation
{
    renderer::Handle<const renderer::RigAnimation> mAnimation = nullptr;
    // Used to get to the map of RigAnimations
    renderer::Handle<const renderer::AnimatedRig> mAnimatedRig = nullptr;
    ...
    float mParameterValue = 0.f; // A float should be enough, we expect animations to
