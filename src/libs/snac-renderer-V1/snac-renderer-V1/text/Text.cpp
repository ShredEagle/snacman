...
math::Vec<2, float> FontData::computeTextSize(const std::string & aString) const
{
    math::Vec<2, float> textSize;
         ...
         /* in body */)
    {
        ...
        textSize.x() += glyph.penAdvance.x();
        textSize.y() = std::max(textSize.y(), glyph.controlBoxSize.height());
