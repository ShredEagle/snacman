...
// TODO: restore this when shadowmaps are available
...
void main(void)
{
...
#ifdef TEXTURES
        * texture(u_DiffuseTexture, vec3(ex_Uv[material.diffuseUvChannel], material.diffuseTextureIndex))
#endif
        ;
    ...
    //
#if defined(TEXTURES) && !defined(NAIVE_TEXTURES)
    vec3 normal_c;
    ...
    if(material.normalUvChannel == 0)
    {
        ...
        vec3 normal_tbn = normalize(
            ...
            (texture(u_NormalTexture, vec3(ex_Uv[material.normalUvChannel], material.normalTextureIndex)).xyz * 2.0 - vec3(1.0))
    ...
    vec3 specular = 
        ub_Light.color * pow(max(0.f, dot(normal_c, h_c)), max(1., material.specularExponent))
    ...
    //phongColor = ambient * albedo.rgb;
    //phongColor = diffuse * albedo.rgb;
    phongColor = specular * albedo.rgb;
